...
Adapted for binaryornot_rs — reads CSVs from the parent Python project
and validates the Rust implementation against the same data.
"""
...
# CSVs live in the original Python project's data directory
_DATA_DIR = Path(__file__).parent.parent.parent / "src" / "binaryornot" / "data"
# CSV test_file paths are relative to the Python project root
...
def load_encoding_rows():
    csv_path = _DATA_DIR / "encodings.csv"
    with csv_path.open(encoding="utf-8") as f:
        return list(csv.DictReader(f))


def load_binary_format_rows():
    csv_path = _DATA_DIR / "binary_formats.csv"
    with csv_path.open(encoding="utf-8") as f:
        return list(csv.DictReader(f))
...
def test_encoding_detected_as_text(row):
    """Text encoded as the given encoding should NOT be classified as binary."""
    ...
    assert binaryornot_rs.is_binary_string(chunk) is False, (
...
def test_encoding_at_min_bytes(row):
    ...
    result = binaryornot_rs.is_binary_string(chunk)
    assert isinstance(result, bool), (
        f"{encoding} at {min_bytes} bytes did not return bool"
    )
...
def test_tiny_ascii_chunks(size):
    ...
    assert binaryornot_rs.is_binary_string(chunk) is False, (
        f"ASCII text at {size} bytes misclassified as binary"
    )
...
def test_tiny_binary_chunks(size):
    ...
    chunk = b"\x00" * size
    assert binaryornot_rs.is_binary_string(chunk) is True, (
        f"All-null data at {size} bytes misclassified as text"
    )
...
def test_mostly_printable_with_high_bytes():
    ...
    for i in range(0, 128, 10):
        chunk[i] = 0xC0 + (i % 30)
    result = binaryornot_rs.is_binary_string(bytes(chunk))
    assert isinstance(result, bool)
...
def test_binary_magic_detected(row):
    ...
    assert binaryornot_rs.is_binary_string(chunk) is True, (
        f"{row['format']} magic bytes misclassified as text"
    )
...
def test_binary_file_detected(row):
    ...
    assert binaryornot_rs.is_binary_string(chunk) is True, (
        f"{row['format']} file {path} misclassified as text"
    )
